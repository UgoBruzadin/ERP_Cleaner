...
function plotERPGUI_pcs(events)
    if nargin < 1
        events = 1;
    end
    ...
    axes6 = axes('Parent', rightPanel, ...
                'Position', [0.1 0.7 0.8 0.2]);

    axes4 = axes('Parent', rightPanel, ...
                'Position', [0.1 0.40 0.35 0.25]);

    axes5 = axes('Parent', rightPanel, ...
...
%                                  'Callback', @interpolateChannels);
    ...
    userData.rightPanel = rightPanel;
...
function updateTopoplots(mainFig, clickedTime, label)
    ...
    [EEG,com] = pop_viewprops_adv_ax(userData.currentEEG, 0, [], 1:10, {'freqrange',[0 55]},{},[],[],[],userData.rightPanel);
