...
function plotERPGUI(events)
    ...
    leftPanel = uipanel('Parent', mainFig, ...
                       'Position', [0 0 0.2 1], ...
                       'BackgroundColor', [0.94 0.94 0.94]);

    % Create panel for side menu
    rightPanel = uipanel('Parent', mainFig, ...
                       'Position', [ 0.85 0 0.15 1], ...
                       'BackgroundColor', [0.94 0.94 0.94]);
    
    % Create panel for plot
    plotPanel = uipanel('Parent', mainFig, ...
                       'Position', [0.125 0 0.725 1]);
    
    % Create two axes for plots
    originalERPaxis = axes('Parent', plotPanel, ...
                'Position', [0.05 0.55 0.4 0.4]);

    previewERPaxis = axes('Parent', plotPanel, ...
                'Position', [0.05 0.05 0.4 0.4]);

    % Create two axes for plots
    originalERPCompaxis = axes('Parent', plotPanel, ...
                'Position', [0.55 0.55 0.4 0.4]);

    previewERPCompaxis = axes('Parent', plotPanel, ...
                'Position', [0.55 0.05 0.4 0.4]);

    compTopoaxis = axes('Parent', rightPanel, ...
                'Position', [0.1 0.8 0.8 0.2]);

    ogTopoaxis = axes('Parent', rightPanel, ...
                'Position', [0.1 0.55 0.8 0.25]);

    prevTopoaxis = axes('Parent', rightPanel, ...
                'Position', [0.1 0.30 0.8 0.25]);

    chanLocaxis = axes('Parent', rightPanel, ...
                'Position', [0.1 0.05 0.8 0.25]);
                   
    % File Selection Section
    createLabel(leftPanel, 'Select EEG File:', 20, 980);
    fileList = dir('*.set');
    ...
    folderButton = uicontrol('Parent', leftPanel, ...
                            ...
                            'Position', [20 920 95 30], ...
                            'Callback', @selectNewDir);
    
    % Event Selection Section
    createLabel(leftPanel, 'Select Event Types:', 20, 900);
    eventList = uicontrol('Parent', leftPanel, ...
                         'Style', 'listbox', ...
                         'String', {'Select a file first'}, ...
                         'Position', [20 840 200 60], ... % Made taller for multiple selections
                         'Max', 2, ... % Enable multiple selection
                         'Value', 1, ...
                         'Callback', @updatePlot);
    
    % Time Window Section
    createLabel(leftPanel, 'Time Window (ms):', 20, 820);
    startTime = uicontrol('Parent', leftPanel, ...
                         'Style', 'edit', ...
                         'String', '-200', ...
                         'Position', [20 790 95 30], ...
                         'Callback', @updatePlot);

    endTime = uicontrol('Parent', leftPanel, ...
                       'Style', 'edit', ...
                       'String', '800', ...
                       'Position', [125 790 95 30], ...
                       'Callback', @updatePlot);
    ...
    selectAll = uicontrol('Parent', leftPanel, ...
                         'Style', 'pushbutton', ...
                         'String', 'Select All', ...
                         'Position', [20 610 95 30], ...
                         'Callback', {@selectChannels, 'all'});
    
    selectNone = uicontrol('Parent', leftPanel, ...
                          'Style', 'pushbutton', ...
                          'String', 'Select None', ...
                          'Position', [125 610 95 30], ...
                          'Callback', {@selectChannels, 'none'});


    % File Selection Section
    createLabel(leftPanel, 'Select second EEG File:', 20, 580);
    fileList = dir('*.set');
    fileNames = {fileList.name};
    fileDropdown = uicontrol('Parent', leftPanel, ...
                            'Style', 'popupmenu', ...
    ...
    createLabel(leftPanel, 'Available Channels/ICs:', 20, 340);
    channelList2 = uicontrol('Parent', leftPanel, ...
                             'Style', 'listbox', ...
                             'String', {'Select a file first'}, ...
                             ...
                             'Callback', @updateTopoplots2);
    % Add forward/backward buttons
    forwardBtn2 = uicontrol('Parent', leftPanel, ...
                          'Style', 'pushbutton', ...
                          'String', '>>', ...
                          'Position', [105 280 30 20], ...
                          'Callback', @moveForward2);
                          
    backwardBtn2 = uicontrol('Parent', leftPanel, ...
                           'Style', 'pushbutton', ...
                           'String', '<<', ...
                           'Position', [105 260 30 20], ...
                           'Callback', @moveBackward2);
                           
    % Add rejected list
    rejectedList2 = uicontrol('Parent', leftPanel, ...
                            'Style', 'listbox', ...
                            'String', {}, ...
    ...
    % Select All/None Buttons for Channels
    selectAll2 = uicontrol('Parent', leftPanel, ...
                         'Style', 'pushbutton', ...
                         'String', 'Select All', ...
                         'Position', [20 210 95 30], ...
                         'Callback', {@selectChannels, 'all'});
    
    selectNone2 = uicontrol('Parent', leftPanel, ...
                          'Style', 'pushbutton', ...
                          'String', 'Select None', ...
...
%     
%     rejectButton = uicontrol('Parent', leftPanel, ...
%                             'Style', 'pushbutton', ...
%                             'String', 'Reject Components', ...
%                             'Position', [20 110 95 30], ...
%                             'Callback', @rejectComponents);
%                             
%     interpolateButton = uicontrol('Parent', leftPanel, ...
%                                  'Style', 'pushbutton', ...
...
%                                  'Callback', @interpolateChannels);
    ...
    %createLabel(leftPanel, 'Save Dataset:', 20, 60);
    ...
    userData.originalERPaxis = originalERPaxis;
    userData.previewERPaxis = previewERPaxis;
    userData.originalERPCompaxis = originalERPCompaxis;
    userData.previewERPCompaxis = previewERPCompaxis;

    userData.chanLocaxis = chanLocaxis;
    userData.ogTopoaxis = ogTopoaxis;
    userData.prevTopoaxis = prevTopoaxis;
    userData.compTopoaxis = compTopoaxis;

    userData.availableList = channelList;
    userData.rejectedList = rejectedList;

    userData.channelList2 = channelList2;
    userData.rejectedList2 = rejectedList2;
...
try 
    ...
    try
        ...
        try
            ...
            items2 = cellstr(num2str((1:numComps)', 'IC%2d'));
            %items2 = 1:size(userData.currentEEG.icaact, 1);
        catch
            items2 = {1:EEG.nbchan};
        end
        
        set(userData.channelList, 'String', items2);
        set(userData.channelList2, 'String', items);
        % Clear Rejected Selection
        set(userData.rejectedList, 'Value', []); % Clear selection
        set(userData.rejectedList, 'String', {});

        set(userData.rejectedList2, 'Value', []); % Clear selection
        set(userData.rejectedList2, 'String', {});
...
function updateChannelList(mainFig)
    ...
    rejectedItems2 = get(userData.rejectedList2, 'String');
    ...
    %else
        % Show channels
        items2 = {userData.currentEEG.chanlocs.labels};
    %end
    
    % Remove any items that are in the rejected list
    items = setdiff(items, rejectedItems, 'stable');
    items2 = setdiff(items2, rejectedItems2, 'stable');
    
    % Update available list
    set(userData.channelList, 'String', items);
    set(userData.channelList2, 'String', items2);
...
function updatePlot(source, ~)
    ...
    cla(userData.originalERPaxis);
    cla(userData.previewERPaxis);
    cla(userData.originalERPCompaxis);
    cla(userData.previewERPCompaxis);
        
    % Plot in both axes
    % For original data (top plot)
    axes(userData.originalERPCompaxis);
    ...
    %    plotComponentERP(userData.originalEEG, selectedEvent, timeWindow, 1:size(userData.originalEEG.icaact, 1));
    %else
    axes(userData.originalERPaxis);
    plotEventERP(userData.originalEEG, selectedEvent, timeWindow, 1:userData.originalEEG.nbchan);
    %end
    title('Original ERP Data');
    ...
    %if get(userData.dataTypeToggle, 'Value') && isfield(tempEEG, 'icaact')
        ...
        axes(userData.previewERPCompaxis);
        ...
        title('Component Preview with Rejections');
    %else
        axes(userData.previewERPaxis);
        plotEventERP(tempEEG, selectedEvent, timeWindow, 1:tempEEG.nbchan);
...
% Update the moveForward function:
function moveForward2(source, ~)
    mainFig = getMainFigure(source);
    userData = get(mainFig, 'UserData');
    
    % Get selected items from available list
    availStr = get(userData.channelList2, 'String');
    availVal = get(userData.channelList2, 'Value');
    ...
    % Get current rejected items
    rejStr = get(userData.rejectedList2, 'String');
    ...
    % Update rejected list
    set(userData.rejectedList2, 'String', rejStr);
...
% Update the moveBackward function:
function moveBackward2(source, ~)
    mainFig = getMainFigure(source);
    userData = get(mainFig, 'UserData');
    
    % Get selected items from rejected list
    rejStr = get(userData.rejectedList2, 'String');
    rejVal = get(userData.rejectedList2, 'Value');
    ...
    % Remove selected items from rejected list
    rejStr(rejVal) = [];
    set(userData.rejectedList2, 'String', rejStr, 'Value', []);
